...
class DeepMicrobiome(object):
    ...
    def run_umaa(self, dims=[64, 32], num_heads=4, epochs=100, batch_size=32, patience=20, loss='mean_squared_error',
                 ae_oact=False, act='relu'):
        self.prefix += 'UMAA_'
        ...
        # Giriş boyutunu dinamik ekle
        dims.insert(0, self.X_train.shape[1])

        # KRİTİK DÜZELTME: self.ae_oact ve self.act yerine doğrudan dışarıdan gelen değişkenleri (ae_oact, act) kullanıyoruz
        self.autoencoder, self.encoder = DNN_models.umaa_tabular(dims, num_heads=num_heads, output_act=ae_oact, act=act)
        ...
        self.autoencoder.fit(self.X_train, self.X_train, epochs=epochs, batch_size=batch_size,
                             callbacks=callbacks, verbose=2, validation_split=0.2)

        # En iyi modeli yükle ve dönüştür
        self.autoencoder = load_model(modelName, compile=False)
        self.encoder = Model(self.autoencoder.input, self.autoencoder.get_layer('umaa_bottleneck').output)

        self.X_train = self.encoder.predict(self.X_train)
        self.X_test = self.encoder.predict(self.X_test)
        print("UMAA Dönüşümü Tamamlandı. Yeni boyutlar:", self.X_train.shape)

    def run_skytu(self, dims=[64, 32], num_heads=4, epochs=100, batch_size=32, patience=20, loss='mean_squared_error',
                  ae_oact=False, act='relu'):
        self.prefix += 'SkyTu_'
        modelName = self.prefix + self.data + '.h5'
        ...
        # Giriş boyutunu (bakteri sayısını) dinamik ekle
        dims.insert(0, self.X_train.shape[1])

        # Modelimizi çağırıyoruz
        self.autoencoder, self.encoder = DNN_models.skytu_autoencoder(dims, num_heads=num_heads, output_act=ae_oact,
                                                                      act=act)
        self.autoencoder.compile(optimizer='adam', loss=loss)
        ...
        # En iyi ağırlıkları geri yükleme
        self.autoencoder = load_model(modelName, compile=False)
        self.encoder = Model(self.autoencoder.input, self.autoencoder.get_layer('skytu_bottleneck').output)

        # Verileri SkyTu alt uzayına izdüşürme
        self.X_train = self.encoder.predict(self.X_train)
        self.X_test = self.encoder.predict(self.X_test)
        print("SkyTu Hibrit Modeliyle Boyut İndirgeme Tamamlandı! Yeni Boyutlar:", self.X_train.shape)
...
if __name__ == '__main__':
    ...
    load_data.add_argument("-d", "--data", help="prefix of dataset to open (e.g. abundance_Cirrhosis)", type=str,
                        choices=["abundance_Cirrhosis", "abundance_Colorectal", "abundance_IBD",
                                 ...
                                 "marker_Obesity", "marker_T2D", "marker_WT2D", "abundance_DrugResponse"
                                 ])
    ...
    parser.add_argument("--skytu", help="run SkyTu Hybrid Attention Autoencoder", action='store_true')
    ...
    parser.add_argument("--umaa", help="run Unsupervised Multi-head Attention Autoencoder", action='store_true')
    parser.add_argument("--num_heads", help="Number of attention heads", type=int, default=4)
    ...
    def run_exp(seed):
        ...
        if args.rp:
            dm.rp()
        if args.umaa:
            dm.run_umaa(dims=[int(i) for i in args.dims.split(',')],
                        num_heads=args.num_heads,
                        epochs=args.max_epochs,
                        batch_size=32,
                        loss=args.aeloss,
                        patience=args.patience,
                        ae_oact=args.ae_oact,  # Parametreyi içeri paslıyoruz
                        act=args.act)  # Parametreyi içeri paslıyoruz
        if args.skytu:
            dm.run_skytu(dims=[int(i) for i in args.dims.split(',')],
                         num_heads=args.num_heads,
                         epochs=args.max_epochs,
                         batch_size=32,
                         loss=args.aeloss,
                         patience=args.patience,
                         ae_oact=args.ae_oact,
                         act=args.act)
